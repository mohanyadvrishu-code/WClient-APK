...
abstract class MappingProvider<T>(val context: Context) {
    ...
    open fun craftMapping(protocolVersion: Int): T {
        val available = availableVersions.filter { it.toInt() <= protocolVersion }
        ...
        if (selectedVersion != protocolVersion.toShort()) {
            Log.w(
                "MappingProvider",
                "No exact mapping for protocol $protocolVersion in $assetPath; " +
                    "falling back to $selectedVersion. New block/item states may be unavailable."
            )
        }
